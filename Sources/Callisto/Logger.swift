...
final class LoggerInfo: @unchecked Sendable {

    // MARK: Nested Types

    /// Log types and their associated ANSI color codes.
    enum LogLevel: String, CaseIterable, Equatable, Hashable {
        case verbose = "⇂"
        case info = ""
        case success = "✓"
        case warning = "⚠"
    ...
    static let shared = LoggerInfo()

    // MARK: Properties

    let supportsColoredOutput: Bool
...
func log(_ message: String, level: LoggerInfo.LogLevel = .info) {
...
func abort(_ error: some Error) -> Never {
